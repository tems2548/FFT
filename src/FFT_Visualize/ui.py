...
Run:
    python ui.py
    python ui.py --wave chirp --freq 20 --freq2 80
    python ui.py --wave sine --freq 440 --samplerate 44100
...
for flashing/logging):
    python ui.py --serial              # pick the port from a GUI list
    python ui.py --serial COM5
    python ui.py --serial /dev/ttyUSB0 --baud 3000000

(FFT.py is kept as a thin backward-compatible launcher -- `python FFT.py`
and `import FFT` both still work exactly as before, delegating here.)
"""
import argparse
import csv
import datetime
...
from protocol import CONNECT_TIMEOUT_S, SerialReader
...
class FFTBenchWindow(QtWidgets.QMainWindow):
    def __init__(self, args, live, reader, port_label, fs):
        ...
        self._trigger_still_needed = None  # None = searching for an edge; int = post-trigger fill countdown
    ...
    @staticmethod
    def _settings_number(s, key, cast):
        """Reads a persisted number, or None if the key was never written.
        ...
        zeroing the caller's setting on a fresh install where the key was
        never written. s.contains() + an explicit cast sidesteps that.
        """
        if not s.contains(key):
            return None
        return cast(s.value(key))
    ...
    def _load_settings(self):
        ...
        averaging = self._settings_number(s, "spectrum/averaging", int)
        ...
        trigger_level = self._settings_number(s, "trigger/level", float)
        if trigger_level is not None:
            self.trigger_level_slider.setValue(int(round(trigger_level * 100)))
        trigger_pretrigger = self._settings_number(s, "trigger/pretrigger_pct", float)
        if trigger_pretrigger is not None:
            self.trigger_pretrigger_slider.setValue(int(round(trigger_pretrigger)))
    ...
    def _build_ui(self):
        ...
        scroll.setWidget(self._build_sidebar())
    ...
    def _build_sidebar(self):
        ...
        logo_label.setPixmap(self.windowIcon().pixmap(30, 30))  # already built in __init__, no need to redraw
    ...
    def _add_plot_card(self, vbox, plot, visible=True):
        """Wraps plot in a card and adds it to vbox with the standard
        stretch factor -- the three lines _build_plots would otherwise
        repeat for every single plot it creates."""
        card = self._wrap_plot_card(plot)
        card.setVisible(visible)
        vbox.addWidget(card, 1)
        return card
    ...
    def _build_plots(self):
        ...
        self.raw_signal_card = self._add_plot_card(vbox, self.raw_signal_plot, visible=self.dsp_lab_mode)
        ...
        self.time_card = self._add_plot_card(vbox, self.time_plot)
        ...
        self.windowed_signal_card = self._add_plot_card(vbox, self.windowed_signal_plot, visible=self.dsp_lab_mode)
        ...
        self.freq_card = self._add_plot_card(vbox, self.freq_plot)
        ...
        self.phase_card = self._add_plot_card(vbox, self.phase_plot)
        ...
        sync_bode_views()
        self.bode_card = self._add_plot_card(vbox, self.bode_plot)
        ...
        self.spec_card = self._add_plot_card(vbox, self.spec_plot)
        ...
        self.noise_card = self._add_plot_card(vbox, self.noise_plot)
        ...
        self.drift_card = self._add_plot_card(vbox, self.drift_plot)
        ...
        self.cepstrum_card = self._add_plot_card(vbox, self.cepstrum_plot)
        ...
        self.goertzel_card = self._add_plot_card(vbox, self.goertzel_plot)
        ...
        self.perf_card = self._add_plot_card(vbox, self.perf_plot)
        ...
        sync_sysres_views()
        self.sysres_card = self._add_plot_card(vbox, self.sysres_plot)
    ...
    def _build_3d_fft_panel(self):
        ...
        if gl is not None:
            try:
                ...
                inner_layout.addWidget(view, 1)
                return self._wrap_plot_card(inner)
            except Exception as exc:
        ...
        return self._wrap_plot_card(placeholder_label)
    ...
    def _on_trigger_mode_change(self, text):
        ...
        self.trigger_armed = True
        self._trigger_still_needed = None
        self.scope_capture = None
        self._update_trigger_controls_enabled()
    ...
    def _update_trigger_status_label(self):
        """Called only on actual state transitions (mode/edge/level/arm
        changes, and from _update_trigger when armed/filling/captured
        changes) rather than every frame -- the text is otherwise unchanged
        30x/sec while a trigger sits idle in one state."""
        ...
        elif self._trigger_still_needed is not None:
    ...
    def _update_trigger(self, display_buffer, n_new):
        ...
        if self.trigger_armed:
            if self._trigger_still_needed is None:
                tail = display_buffer[-n_new:]
                level = self.trigger_level
                # Boundary transition (previous frame's last sample -> this
                # frame's first new sample) is checked as a scalar rather
                # than folded into one array via np.concatenate -- tail[:-1]/
                # tail[1:] below are zero-copy views, so this avoids any
                # per-frame allocation in the common (no trigger yet) case.
                if self.trigger_edge == "Rising":
                    boundary_cross = self._trigger_prev_sample < level <= tail[0]
                    interior = np.flatnonzero((tail[:-1] < level) & (tail[1:] >= level))
                else:
                    boundary_cross = self._trigger_prev_sample > level >= tail[0]
                    interior = np.flatnonzero((tail[:-1] > level) & (tail[1:] <= level))
                if boundary_cross:
                    ...
                    local_idx = int(interior[0]) + 1  # +1: interior[j] flags the transition landing on tail[j+1]
                else:
                    local_idx = None
